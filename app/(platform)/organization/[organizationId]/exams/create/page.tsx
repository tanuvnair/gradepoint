        ...
        for (const [sectionIndex, section] of formData.sections.entries()) {
            ...
            for (const [
                ...
                if (!question.content.trim()) {
                    toast({
                        ...
                        description: `Question ${
                            questionIndex + 1
                        } in section "${section.title}" is required`,
                    });
                    return false;
                }
                ...
                if (question.type === "MULTIPLE_CHOICE") {
                    const hasOptions = Object.values(
                        question.options || {}
                    ).some((option) => option?.toString().trim());
                    if (!hasOptions) {
                        toast({
                            variant: "destructive",
                            title: "Validation Error",
        ...
        async (e: React.FormEvent) => {
            ...
            try {
                ...
                const response = await fetch(
                    `/api/organization/${organizationId}/exams`,
                    {
                        method: "POST",
                        headers: {
    ...
    const handleSaveAsDraft = useCallback(async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    error.message || "Failed to save exam as draft"
                );
            }
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground sm:text-base">
                        Configure your exam settings and questions
                    </p>
                </div>
            </div>
            {memoizedForm}
